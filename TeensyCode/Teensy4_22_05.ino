...
void loop() {
   ...
   // Serial.print(type);
    ...
    //-------------------------------write to CUTOFF---------------------------------//
    CUTOFF=int(cutoff_position*(adsr1_sample*CUTOFF_ADSR1_AMOUNT + lfo1_sample*CUTOFF_LFO1_AMOUNT) + 2048); //all modulations weighted and added together 
    if (CUTOFF > 4095){
        CUTOFF=4095;
    }
    ...
    //-------------------------------write to RES---------------------------------//
    RES=int(res_position*(adsr1_sample*RES_ADSR1_AMOUNT + lfo1_sample*RES_LFO1_AMOUNT) + 2048); //all modulations weighted and added together 
    if (RES > 4095){
        RES=4095;
    }
    ...
    myDacG.analogWrite(1,VCA6); 

    //-------------------------------write to OUTPUT VCA---------------------------------//
    VCA8=int(volume_position*(adsr1_sample+2048)*(velocity/127)); //hard coded adsr1 to control volume enveloppe at all time, no volume mod coded for this iteration
    if (VCA8 > 4095){
        VCA8=4095;
    }
