...
#define ID_VCO1_SHAPE_ADSR1_AMOUNT 38
#define ID_VCO1_SHAPE_LFO1_AMOUNT 138
...
float           CUTOFF_ADSR1_AMOUNT = 0.5;
float           CUTOFF_LFO1_AMOUNT = 0.;

float           RES_ADSR1_AMOUNT = 0.;
float           RES_LFO1_AMOUNT = 0.2;


float           VCO1_SHAPE_ADSR1_AMOUNT = 0.;
float           VCO1_SHAPE_LFO1_AMOUNT = 0.5;
...
//  myDacB.begin();
  ...
  lfo1.setMode0Freq(1);
  lfo1.setAmpl(DACSIZE);
  // init amplitude
  
  adsr1.setAttack(1000);
  adsr1.setDecay(1000);
  adsr1.setRelease(1000);
  adsr1.setAttack(1000); 
   ...
   // Serial.print(type);
    ...
    case usbMIDI.NoteOn:
        ...
        if (note_priority == 1){ // Top note priority
          mV=commandTopNote(channel,adsr_trig, adsr_trig_prev );
          myDacA.analogWrite(0,mV);
          
          if (adsr_trig != adsr_trig_prev && adsr_trig==1){
              adsr1.noteOn(t);
          if (adsr_trig != adsr_trig_prev && adsr_trig==0){
            adsr1.noteOff(t);
          }            
            }
        }
        else if (note_priority == 0){ // Bottom note priority
          mV= commandBottomNote(channel,adsr_trig,adsr_trig_prev);
          myDacA.analogWrite(0,mV);
          if (adsr_trig != adsr_trig_prev && adsr_trig==1){
            adsr1.noteOn(t);
          }
         ...
         mV = commandLastNote(channel,adsr_trig,adsr_trig_prev);
         myDacA.analogWrite(0,mV);
         if (adsr_trig != adsr_trig_prev && adsr_trig==1){
            adsr1.noteOn(t);
         }
    ...
    VCA4=int(cos(VCO1_SHAPE+adsr1_sample*VCO1_SHAPE_ADSR1_AMOUNT + lfo1_sample*VCO1_SHAPE_LFO1_AMOUNT)*(lvl1_position + adsr1_sample*LVL1_ADSR1_AMOUNT + lfo1_sample*LVL1_LFO1_AMOUNT )); 
    ...
    VCA5=int(sin(VCO1_SHAPE+adsr1_sample*VCO1_SHAPE_ADSR1_AMOUNT + lfo1_sample*VCO1_SHAPE_LFO1_AMOUNT)*(lvl1_position + adsr1_sample*LVL1_ADSR1_AMOUNT + lfo1_sample*LVL1_LFO1_AMOUNT )); 
    if (VCA5 > 4095){
        VCA5=4095;
    }
    if (VCA5 < 0 ){
  ...
  if (Serial.available()) {
    ...
    switch (rx_state) {
      ...
      case 4:                     // fourth is the least significant byte of the value
        ...
        else if (cc_control == ID_VCO1_SHAPE_ADSR1_AMOUNT){
            VCO1_SHAPE_ADSR1_AMOUNT = float((value-50)/100) ;
        }
        else if (cc_control == ID_VCO1_SHAPE_LFO1_AMOUNT){
            VCO1_SHAPE_LFO1_AMOUNT = float((value-50)/100) ;
        }
...
int commandTopNote(int channel,bool adsr_trig, bool adsr_trig_prev){
  int topNote = 0;
  ...
  // turn adsrs off
     adsr_trig_prev = adsr_trig;
     adsr_trig = LOW;

  }
}
int commandBottomNote(int channel, bool adsr_trig, bool adsr_trig_prev){
...
int commandLastNote(int channel, bool adsr_trig, bool adsr_trig_prev ){
