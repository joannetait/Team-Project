// This is a test for the encoders. The code updates the encoders using Noe's function
// then uses Tomek's functions to print the values to the serial monitor.
// 
// To use:
// Load this code onto the Arduino.
//
// Open serial monitor, the words: "starting encoder test..." will appear.
// If this does appear, the code has not uploaded correctly!
// 
// When the encoders are turned, the new values will be printed to the serial monitor
//
// The main loop is at line 430, the function for checking and printing one encoder is at line 2178
//
// Of less interest, the function for checking and printing all the encoders is at 2064.
...
void setup(){
    ...
    Serial.begin(9600);
    Serial3.begin(9600);
    Serial.print("starting encoder test...");
    ...
    if (VCO_select == LOW) { //encoders control VCO1
        if (VCO1_OCT_LVL_select == LOW) { //encoder controls octave number
            ...
            if (VCO1_OCT_position != VCO1_OCT_position_prev){
                PrintEnc("VCO1 OCT", VCO1_OCT_position);
        ...
        if (VCO1_OCT_LVL_select == HIGH) { //encoder controls VCO1 volume
            ...
            if (VCO1_LVL_position != VCO1_LVL_position_prev){
                PrintEnc("VCO1 LVL", VCO1_LVL_position);
    ...
    if (VCO_select == HIGH) { //encoders control VCO2
        if (VCO2_OCT_LVL_select == LOW) { //encoder controls octave number
            ...
            if (VCO2_OCT_position != VCO2_OCT_position_prev){
                PrintEnc("VCO2 OCT", VCO2_OCT_position);
                //send change to teensy
        ...
        if (VCO2_OCT_LVL_select == HIGH) { //encoder controls VCO1 volume
            ...
            if (VCO2_LVL_position != VCO2_LVL_position_prev){
                PrintEnc("VCO1 LVL", VCO2_LVL_position);
...
// This function calls on EncMonitor for each encoder variable
// See below for a detailing of the the EncMonitor function
void monitorEncoders(){
    EncMonitor("VCO1 OCT", VCO1_OCT_position, VCO1_OCT_position_prev);
    EncMonitor("VCO1 LVL", VCO1_LVL_position, VCO1_LVL_position_prev);
    EncMonitor("VCO1 FM", VCO1_FM_AMOUNT_position, VCO1_FM_AMOUNT_position_prev);
    EncMonitor("VCO1 SHAPE", VCO1_SHAPE_position, VCO1_SHAPE_position_prev);
    EncMonitor("VCO1 SHAPE", VCO1_PWM_position, VCO1_PWM_position_prev);

    EncMonitor("VCO2 OCT", VCO2_OCT_position, VCO2_OCT_position_prev);
    EncMonitor("VCO2 LVL", VCO2_LVL_position, VCO2_LVL_position_prev);
    EncMonitor("VCO2 DETUNE", VCO2_DETUNE_position, VCO2_DETUNE_position_prev);
    EncMonitor("VCO2 SHAPE", VCO2_SHAPE_position, VCO2_SHAPE_position_prev);
    EncMonitor("VCO2 SHAPE", VCO2_PWM_position, VCO2_PWM_position_prev);

    EncMonitor("CUTOFF", CUTOFF_position, CUTOFF_position_prev);
    EncMonitor("RES", RES_position, RES_position_prev);
    EncMonitor("DRIVE", DRIVE_position, DRIVE_position_prev);

    EncMonitor("LFO1 FREQ", LFO1_FREQ_position, LFO1_FREQ_position_prev);
    EncMonitor("LFO1 RATE", LFO1_RATE_position, LFO1_RATE_position_prev);
    EncMonitor("LFO1 SHAPE", LFO1_SHAPE_position, LFO1_SHAPE_position_prev);
    EncMonitor("LFO1 PHASE", LFO1_PHASE_position, LFO1_PHASE_position_prev);
    EncMonitor("LFO1 AMP", LFO1_AMPLITUDE_position, LFO1_AMPLITUDE_position_prev);
    EncMonitor("LFO1 OFFSET", LFO1_OFFSET_position, LFO1_OFFSET_position_prev);

    EncMonitor("LFO2 FREQ", LFO2_FREQ_position, LFO2_FREQ_position_prev);
    EncMonitor("LFO2 RATE", LFO2_RATE_position, LFO2_RATE_position_prev);
    EncMonitor("LFO2 SHAPE", LFO2_SHAPE_position, LFO2_SHAPE_position_prev);
    EncMonitor("LFO2 PHASE", LFO2_PHASE_position, LFO2_PHASE_position_prev);
    EncMonitor("LFO2 AMP", LFO2_AMPLITUDE_position, LFO2_AMPLITUDE_position_prev);
    EncMonitor("LFO2 OFFSET", LFO2_OFFSET_position, LFO2_OFFSET_position_prev);

    EncMonitor("ADSR1 ATTACK", ADSR1_ATTACK_position, ADSR1_ATTACK_position_prev);
    EncMonitor("ADSR1 DECAY", ADSR1_DECAY_position, ADSR1_DECAY_position_prev);
    EncMonitor("ADSR1 SUSTAIN", ADSR1_SUSTAIN_position, ADSR1_SUSTAIN_position_prev);
    EncMonitor("ADSR1 RELEASE", ADSR1_RELEASE_position, ADSR1_RELEASE_position_prev);

    EncMonitor("ADSR2 ATTACK", ADSR2_ATTACK_position, ADSR2_ATTACK_position_prev);
    EncMonitor("ADSR2 DECAY", ADSR2_DECAY_position, ADSR2_DECAY_position_prev);
    EncMonitor("ADSR2 SUSTAIN", ADSR2_SUSTAIN_position, ADSR2_SUSTAIN_position_prev);
    EncMonitor("ADSR2 RELEASE", ADSR2_RELEASE_position, ADSR2_RELEASE_position_prev);

    EncMonitor("LFO1 DEST1", LFO1_DEST1_position, LFO1_DEST1_position_prev);
    EncMonitor("LFO2 DEST1", LFO2_DEST1_position, LFO2_DEST1_position_prev);
    EncMonitor("ADSR1 DEST1", ADSR1_DEST1_position, ADSR1_DEST1_position_prev);
    EncMonitor("ADSR2 DEST1", ADSR2_DEST1_position, ADSR2_DEST1_position_prev);

    EncMonitor("LFO1 DEST2", LFO1_DEST2_position, LFO1_DEST2_position_prev);
    EncMonitor("LFO2 DEST2", LFO2_DEST2_position, LFO2_DEST2_position_prev);
    EncMonitor("ADSR1 DEST2", ADSR1_DEST2_position, ADSR1_DEST2_position_prev);
    EncMonitor("ADSR2 DEST2", ADSR2_DEST2_position, ADSR2_DEST2_position_prev);

    EncMonitor("LFO1 DEST3", LFO1_DEST3_position, LFO1_DEST3_position_prev);
    EncMonitor("LFO2 DEST3", LFO2_DEST3_position, LFO2_DEST3_position_prev);
    EncMonitor("ADSR1 DEST3", ADSR1_DEST3_position, ADSR1_DEST3_position_prev);
    EncMonitor("ADSR2 DEST3", ADSR2_DEST3_position, ADSR2_DEST3_position_prev);

    EncMonitor("LFO1 DEST4", LFO1_DEST4_position, LFO1_DEST4_position_prev);
    EncMonitor("LFO2 DEST4", LFO2_DEST4_position, LFO2_DEST4_position_prev);
    EncMonitor("ADSR1 DEST4", ADSR1_DEST4_position, ADSR1_DEST4_position_prev);
    EncMonitor("ADSR2 DEST4", ADSR2_DEST4_position, ADSR2_DEST4_position_prev);

    EncMonitor("LFO1 DEST5", LFO1_DEST5_position, LFO1_DEST5_position_prev);
    EncMonitor("LFO2 DEST5", LFO2_DEST5_position, LFO2_DEST5_position_prev);
    EncMonitor("ADSR1 DEST5", ADSR1_DEST5_position, ADSR1_DEST5_position_prev);
    EncMonitor("ADSR2 DEST5", ADSR2_DEST5_position, ADSR2_DEST5_position_prev);

    EncMonitor("LFO1 DEST6", LFO1_DEST6_position, LFO1_DEST6_position_prev);
    EncMonitor("LFO2 DEST6", LFO2_DEST6_position, LFO2_DEST6_position_prev);
    EncMonitor("ADSR1 DEST6", ADSR1_DEST6_position, ADSR1_DEST6_position_prev);
    EncMonitor("ADSR2 DEST6", ADSR2_DEST6_position, ADSR2_DEST6_position_prev);

    EncMonitor("LFO1 DEST7", LFO1_DEST7_position, LFO1_DEST7_position_prev);
    EncMonitor("LFO2 DEST7", LFO2_DEST7_position, LFO2_DEST7_position_prev);
    EncMonitor("ADSR1 DEST7", ADSR1_DEST7_position, ADSR1_DEST7_position_prev);
    EncMonitor("ADSR2 DEST7", ADSR2_DEST7_position, ADSR2_DEST7_position_prev);

    EncMonitor("LFO1 DEST8", LFO1_DEST8_position, LFO1_DEST8_position_prev);
    EncMonitor("LFO2 DEST8", LFO2_DEST8_position, LFO2_DEST8_position_prev);
    EncMonitor("ADSR1 DEST8", ADSR1_DEST8_position, ADSR1_DEST8_position_prev);
    EncMonitor("ADSR2 DEST8", ADSR2_DEST8_position, ADSR2_DEST8_position_prev);

    EncMonitor("LFO1 DEST9", LFO1_DEST9_position, LFO1_DEST9_position_prev);
    EncMonitor("LFO2 DEST9", LFO2_DEST9_position, LFO2_DEST9_position_prev);
    EncMonitor("ADSR1 DEST9", ADSR1_DEST9_position, ADSR1_DEST9_position_prev);
    EncMonitor("ADSR2 DEST9", ADSR2_DEST9_position, ADSR2_DEST9_position_prev);

    EncMonitor("LFO1 DEST10", LFO1_DEST10_position, LFO1_DEST10_position_prev);
    EncMonitor("LFO2 DEST10", LFO2_DEST10_position, LFO2_DEST10_position_prev);
    EncMonitor("ADSR1 DEST10", ADSR1_DEST10_position, ADSR1_DEST10_position_prev);
    EncMonitor("ADSR2 DEST10", ADSR2_DEST10_position, ADSR2_DEST10_position_prev);

    EncMonitor("LFO1 DEST11", LFO1_DEST11_position, LFO1_DEST11_position_prev);
    EncMonitor("LFO2 DEST11", LFO2_DEST11_position, LFO2_DEST11_position_prev);
    EncMonitor("ADSR1 DEST11", ADSR1_DEST11_position, ADSR1_DEST11_position_prev);
    EncMonitor("ADSR2 DEST11", ADSR2_DEST11_position, ADSR2_DEST11_position_prev);

    EncMonitor("LFO1 DEST12", LFO1_DEST12_position, LFO1_DEST12_position_prev);
    EncMonitor("LFO2 DEST12", LFO2_DEST12_position, LFO2_DEST12_position_prev);
    EncMonitor("ADSR1 DEST12", ADSR1_DEST12_position, ADSR1_DEST12_position_prev);
    EncMonitor("ADSR2 DEST12", ADSR2_DEST12_position, ADSR2_DEST12_position_prev);
}


//-----------Function to monitor one encoder-------//
// This function takes the current and previous values of each 
// encoder variable and prints to serial monitor if the variable
// has changed.
// Since the "previous value" is updated from previous at the start of each if statement 
// in the updateEncoders function, it will not be updated yet at the end 
// of the main loop, so we can run this function for each encoder after
// all the encoders have been checked.
//
// (Noe - forget my comments about this yesterday evening)

void EncMonitor(const char *enc, int value, int prev){
    if(value != prev){                      //if the encoder value has changed   
        Serial.print("encoder ");
        Serial.print(enc);                  //print encoder name
        Serial.print(" is at: ");
        Serial.print(value);                //print encoder value
    }
}
//------------end of EncMonitor function--------//



//------unused function---------//
void PrintEnc(const char *enc, int value){
    Serial.print("encoder ");
    Serial.print(enc);
    Serial.print(" is at: ");
    Serial.print(value);
